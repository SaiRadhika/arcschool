...
public class WaitHelper extends BaseClass {
	...
	public void WaitForElementVisibleWithPollingTime(WebElement element, int TimoutInSeconds,
			int PollingEveryMillSeconds) {
	...
	private Wait<WebDriver> getFluentWait(int TimoutInSeconds, int PollingEveryMillSeconds) {
		Wait<WebDriver> fwait = new FluentWait<WebDriver>(driver).withTimeout(Duration.ofSeconds(TimoutInSeconds))
